import { blake2b, concat, Hex } from '@tzstamp/helpers'
import { Relation, Path } from './path'
...
export class MerkleTree {
   ...
   */
  *paths (index: number): Generator<Path> {
    for (const index in this.#layers[0]) {
      ...
      // Path steps
      const steps: [Relation, Uint8Array][] = []
