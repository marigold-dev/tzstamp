...
async function buildHighProof (block, opHash, root) {
  const opGroup = block.operations[0][3].find((opGroup) => {
    opGroup.hash == opHash
  })
  if (!opGroup) {
  ...
  const blockHashProof = buildBlockHashProof(block.header)
  return opGroupProof
    .concat(opsHashProof)
...
/**
 * Builds a proof committing the aggregator root the operation hash.
 *
 * @param {object} opGroup Operation group data
 * @param {Uint8Array} root Aggregator root
 */
function buildOpGroupProof (opGroup, root) {
  const [ revealOp, txnOp ] = separateOperations()
...
/**
 * Builds the proof committing the operation hash to the fourth operation pass hash.
 *
 ...
 */
function buildBlockHashProof (header) {
